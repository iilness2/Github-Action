...
resource "aws_cloudwatch_metric_alarm" "instances_cpu_high" {
  ...
  threshold           = var.instances_cpu_high_threshold
  period              = "300"
  statistic           = "Average"
  ...
  alarm_name          = "${var.asg_name}-instances-CPU-Utilization-Above-${var.instances_cpu_high_threshold}"
...
resource "aws_cloudwatch_metric_alarm" "instances_cpu_low" {
  evaluation_periods  = "1"
  threshold           = var.instances_cpu_low_threshold
  period              = "300"
  statistic           = "Average"
  metric_name         = "CPUUtilization"
  namespace           = var.namespace
  ...
  alarm_name          = "${var.asg_name}-instances-CPU-Utilization-Below-${var.instances_cpu_low_threshold}"
